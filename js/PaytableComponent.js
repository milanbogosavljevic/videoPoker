...
(function(){
    ...
    p.init = function (paytableArr) {
        ...
        const bonusWinLabel = system.CustomMethods.makeText('BONUS WIN', '36px Russo One', '#dcf799', 'left', 'hanging');
        bonusWinLabel.x = 136;
        bonusWinLabel.y = 428;
        system.CustomMethods.cacheText(bonusWinLabel);
        this.addChild(bonusWinLabel);

        const bonusWin = this._bonusWin = system.CustomMethods.makeText('0.00', '34px Russo One', '#dcf799', 'center', 'middle');
        bonusWin.x = 560;
        bonusWin.y = 443;
    ...
    p.updateForWin = function(win) {
        if(win === 'noWin'){
            ...
            this._winMarker.y = paytableWin.y - 11;
