...
(function(){
    ...
    p._setTextFields = function() {
        const betTextLabel = system.CustomMethods.makeText('BET', '31px Russo One', '#f8fffd', 'center', 'middle');
        betTextLabel.x = 360;
        betTextLabel.y = 30;
        ...
        for(let i = 0; i < this._betLevels.length; i++){
            let betValue = system.CustomMethods.makeText(this._betLevels[i], '24px Russo One', '#5b8bff', 'center', 'middle');
        ...
        const winTextlabel = system.CustomMethods.makeText('WIN', '31px Russo One', '#f8fffd', 'center', 'middle');
        winTextlabel.x = 960;
        winTextlabel.y = 30;
        system.CustomMethods.cacheText(winTextlabel);

        let winValue = this._winText = system.CustomMethods.makeText('0.00', '36px Russo One', '#ec5750', 'center', 'middle');
        winValue.x = winTextlabel.x;
        winValue.y = 103;
        system.CustomMethods.cacheText(this._winText);

        const balanceTextLabel = system.CustomMethods.makeText('BALANCE', '31px Russo One', '#f8fffd', 'center', 'middle');
        balanceTextLabel.x = 1560;
        balanceTextLabel.y = 30;
        system.CustomMethods.cacheText(balanceTextLabel);

        let balanceValue = this._balanceText = system.CustomMethods.makeText('0', '36px Russo One', '#ec5750', 'center', 'middle');
    ...
    p._setButtons = function() {
        ...
        bet.x = 128;
        bet.y = -50;
        const betText = system.CustomMethods.makeText('BET', '31px Russo One', '#3f3f3f', 'center', 'middle');
    ...
    p._moveBetMarker = function() {
        this._betMarker.x = 244 + (this._currentBetIndex * 58);
    };
